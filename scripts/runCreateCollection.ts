...
import { uploadMetadata } from "./uploadCNFTMetadata";
dotenv.config();
...
//   // const HELIUS_URL = `https://devnet.helius-rpc.com/?api-key=${process.env.HELIUS_API_KEY}`
  ...
  const payer = Keypair.fromSecretKey(Uint8Array.from(Buffer.from(process.env.COLLECTION_OWNER_SECRET_KEY as string, "base64")));
  ...
  // const uri = uploadMetadata(metaplex, 1)
